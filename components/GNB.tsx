...
export default function GNB() {
    const menuItems = [
        ...
        },
        {
            title: "관리비조회",
            subItems: [
              { title: "우리집관리비", href: `/fee/my?year=${new Date().getFullYear()}&month=${new Date().getMonth() + 1}` },
              { title: "관리비상세보기", href: `/fee/detail?year=${new Date().getFullYear()}&month=${new Date().getMonth() + 1}` },
            ],
        },
      ];
    ...
    return (
        <div className={`${state? "visible": "invisible"} z-10 absolute w-full bg-grey_50 shadow-xl`}>
            <div className="flex px-4 max-w-[1200px] m-auto">
                <span className="w-[124px]"></span>
                <div className="flex px-4 flex-1 justify-center">
                {menuItems.map((item, index) => (
                    <ul key={`${item}_${index}`}className="w-full text-center">
                        {item.subItems.map((subItem, subIndex) => (
                        <li key={`${subItem}_${subIndex}`} className="py-3.5 hover:bg-main_color hover:text-white whitespace-nowrap">
                            <Link
                                href={subItem.href}>
                                {subItem.title}
                            </Link>
