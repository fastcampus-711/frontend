...
export default function ComplainDetail({
  ...
  const handleReactionGood = async () => {
    if (reaction === "GOOD") {
      ...
      try {
        const response = await fetch(
          "https://711.ha-ving.store/reactions/post",
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          const responseData = await response.json()
          console.log("반응 취소 성공:", responseData)
          ...
          console.error("반응 취소를 실패했습니다.:", response.statusText)
        }
      } catch (error) {
        console.error("에러 발생:", error)
      }
  ...
  const handleDelete = async () => {
    if (confirm("게시글을 삭제하시겠습니까?")) {
      try {
        const response = await fetch(
          `https://711.ha-ving.store/boards/complains/${id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <div>
      <div className="flex gap-1 pb-6 text-grey_600 text-lg">
        <span>민원게시판</span>
        <span>&gt;</span>
        <span className="text-grey_900">전체민원</span>
      </div>
